...
4. Perform pairwise regression with all remaining strands as the independent variate and the regression from the previous iteration, repeating Steps 1b-2.
5. The strand which elicits the lowest concentration measure is again chosen.
6. Repeat Steps 4-5 until all strands have been regressed, resulting in a single consensus seration.

The use of linear regression has its virtue in that partial rankings which may be identical in their relative order but which may run in reverse sequence, will be regressed into the same order but merely with a negative slope.

Given that the order in which strands are regressed has a bearing on the resulting consensus seriation, the use of the concentration principle to determine that order is necessary. Especially if the number of strands were to exceed 10, all possible permutations of the order of strand regression could not be evaluated in a reasonable amount of time. Similarly, PCA (which was initially explored as a method of harmonizing rankings) would only work as a method of ranking imputation if there were at least four elements jointly shared across all strands, a restriction which was undesirable given a potential need to perform consensus seration on a large set of data in piecemeal fashion.
...
Here, we can can create an object that contains the seriated incidence matrices, according to the results of Lakhesis analysis:
```{r}
...
```
Lower concentration measures inidicate more optimal seriations. As can be seen, the Lakhesis analysis has arrived at a more optimal seriation according to the concentration principle.
