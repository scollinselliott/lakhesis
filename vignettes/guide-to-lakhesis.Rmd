...
The `lakhesis` package, named for the ancient Greek goddess who measured the thread of one's life, provides this functionality, to explore and select well seriated sequences as "strands" that are then "threaded" togeher into a single consensus seriation. The package can either be installed from (CRAN)[https://CRAN.R-project.org/package=lakhesis], or from (GitHub)[https://github.com/scollinselliott/lakhesis], the latter of which hosts the most recent development version. To install from GitHub, the library `devtools` is needed:
```{r, eval  =FALSE}
library(devtools)
install_github("scollinselliott/lakhesis") 
```

This vignette showcases the primary functions of the package side-by-side with the Lakhesis Calucator, a `shiny` app whose interface offers the means to perform and critically examine seriated sequences from an input. Command-line functions to prepare 

## Getting Started: Formating Data
...
* `"im_serated"` produces a matrix plot of the seriated incidence matrix, showing the measure of concentration $\kappa$ (on which see below).
...
The resulting incidence matrix itself may be extracted from the `strand` by calling `$im_seriated`:
...
In the process of exploring matrices for seriated sequences, an investigator is liable to come across many sequences which appear to be well seriated. The question then arises as to how to harmonize or reconcile several partial, seriated sequencess, some or may of which may pose disagreement. This is a non-trivial problem analogous to other problems involving discrepant and/or partial ranking but is not identical given the goal is a consensus seriation.
...
3. Restore the elements which were temporarily suppressed in either strand, and perform simple linear regression separately for either strand onto that best fitting line.
  a. If the same element in eiher strand is regressed onto different points along the best fitting line, the mean of their regression is used.
  b. The elements are re-ranked along the regressison line, providing a single "harmonized" ranking from the two strands.
...
The selection of strands is performed by the investigator via the Lakhesis Calculator (on which see below), but a predefined `strands` object is included in the data of the package to illustrate functionality of the `lakhesize()` function. The `qfStrands` object contains three strands from the `quattrofontanili` data:
...
As discussed in the paper at the start of this vignette, there are a number of optimality measures for seriated matrices. Here, a measure of concentration has been defined, $\kappa$, which is a loss measure with a minimum of 1. Concentration measures the number of non-contiguous values of 1 attested in the seriated matrix across both columns and rows. The intervals of the index from the lowest-ranked 1 and that of the highest ranked 1 is summed for each row and column, and then divided by twice total sum of the matrix. A seriated matrix that has perfect concentration will thus have $\kappa = 1$, and should there occur 0s in between values of 1, the measure of $\kappa$ will increase. This is performed using the function `conc_kappa()`, whose documentation contains more information, which takes as input an incidence matrix.
...
Plotting the results of `lakhesize()` can be accomplished by stipulating different options for the `display` argument in a `plot()` function, taking a `lakhesis` object as input. The default

* `"im_seriated"` will produce a matrix plot of the consensus seriation, also displaying the concentration measure $\kappa$
...
* `"agreement"` will produce a bar graph showing the measures of agreement of each strand with the consensus. Higher values indicate better agreeement.
...
The commands above can all be used in the console, and, as recommended, performing `lakhesize()` with more simulations will help to ensure that the most optimal consensus seration will be identified. That said the functionality of the `lakhesis` package is best articulated via the interactive Lakhesis Calculator, written using `shiny`, which is initialized by the function `LC()`:
```{r, eval = FALSE}
LC()
```
...
* `$consensus` An object of the `lakhesis` class that contains the information on the consensus seration (see above on the `lakhesize()` function)
