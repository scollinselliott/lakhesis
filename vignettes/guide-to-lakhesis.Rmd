...
First, one can create two objects of the seriated incidence matrices, one from the results of Lakhesis analysis using `"exploratory"` option (`L.exp`) and the other using the `"optimize"` option:
```{r}
quattrofontanili.L <- L$im_seriated
```
Then, one can assess the concentration measure $\kappa$ for each seriated matrix:
```{r}
